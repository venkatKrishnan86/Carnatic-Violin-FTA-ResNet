...
class FTA_Module(nn.Module):
    def __init__(self, shape, kt, kf):
        """
            shape: (freq_bin, time_segment, start_num_channels, final_num_channels)
            kt: Number of kernels/filters for time branch
            kf: Number of kernels/filters for frequency branch
        """
...
class OriginalFTAnet(nn.Module):
    def __init__(self, freq_bin = 360, time_segment = 128):
        super(OriginalFTAnet, self).__init__()
...
class DeeperFTAnet(nn.Module):
    def __init__(self, freq_bin = 360, time_segment = 128):
        super(DeeperFTAnet, self).__init__()
        self.bn_layer = nn.BatchNorm2d(3)
        # bm (Melody branch)
        self.bm_layer = nn.Sequential(
            nn.Conv2d(3, 16, (4,1), stride=(4,1)),
        ...
        self.fta_3 = FTA_Module((freq_bin // 4, time_segment // 4, 64, 128), 3, 3)
        self.fta_4 = FTA_Module((freq_bin // 4, time_segment // 4, 128, 128), 3, 3)
        self.fta_4_1 = FTA_Module((freq_bin // 4, time_segment // 4, 128, 128), 3, 3)
        self.fta_5 = FTA_Module((freq_bin // 2, time_segment // 2, 128, 64), 3, 3)
        self.fta_6 = FTA_Module((freq_bin, time_segment, 64, 32), 3, 3)
        self.fta_7 = FTA_Module((freq_bin, time_segment, 32, 1), 3, 3)
        ...
        self.sf_3 = SF_Module(3, 128, 4, 4)
        self.sf_4 = SF_Module(3, 128, 4, 4)
        self.sf_4_1 = SF_Module(3, 128, 4, 4)
        self.sf_5 = SF_Module(3, 64, 4, 4)
        self.sf_6 = SF_Module(3, 32, 4, 4)
        self.sf_7 = SF_Module(3, 1, 4, 4)
    ...
    def forward(self, x):
        ...
        x_r, x_t, x_f = self.fta_1(x)
        x = self.sf_1([x_r, x_t, x_f]) # (32, 320, 128)

        x = self.mp_1(x) # (32, 160, 64)

        x_r, x_t, x_f = self.fta_2(x)
        x = self.sf_2([x_r, x_t, x_f]) # (64, 160, 64)

        x = self.mp_2(x) # (64, 80, 32)

        x_r, x_t, x_f = self.fta_3(x)
        x = self.sf_3([x_r, x_t, x_f]) # (128, 160, 64)

        x_r, x_t, x_f = self.fta_4(x)
        x = self.sf_4([x_r, x_t, x_f])

        x_r, x_t, x_f = self.fta_4_1(x) # New line
        x = self.sf_4_1([x_r, x_t, x_f]) # New line
...
class FTAnet(nn.Module):
    def __init__(self, freq_bin = 360, time_segment = 128):
        ...
        self.fta_3 = FTA_Module((freq_bin // 4, time_segment // 4, 64, 128), 3, 3)
        self.fta_4 = FTA_Module((freq_bin // 4, time_segment // 4, 128, 128), 3, 3)
        self.fta_4_1 = FTA_Module((freq_bin // 4, time_segment // 4, 128, 128), 3, 3) # New line
        self.fta_5 = FTA_Module((freq_bin // 2, time_segment // 2, 128, 64), 3, 3)
        self.fta_6 = FTA_Module((freq_bin, time_segment, 64, 32), 3, 3)
        self.fta_7 = FTA_Module((freq_bin, time_segment, 32, 1), 3, 3)
        ...
        self.sf_3 = SF_Module(3, 128, 4, 4)
        self.sf_4 = SF_Module(3, 128, 4, 4)
        self.sf_4_1 = SF_Module(3, 128, 4, 4) # New line
        self.sf_5 = SF_Module(3, 64, 4, 4)
        self.sf_6 = SF_Module(3, 32, 4, 4)
        self.sf_7 = SF_Module(3, 1, 4, 4)
    ...
    def forward(self, x):
        ...
        x_r, x_t, x_f = self.fta_5(x + self.up_3(x2))
        x = self.sf_5([x_r, x_t, x_f])
        x = self.up_2(x)
        ...
        x_r, x_t, x_f = self.fta_7(x + self.up_4(x1))
        x = self.sf_7([x_r, x_t, x_f])
