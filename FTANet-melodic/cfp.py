...
def STFT(x, fr, fs, Hop, h):        
    ...
    tfr = abs(scipy.fftpack.fft(tfr, n=N, axis=0))  # Magnitude Response
    return tfr, f, t, N
...
def feature_extraction(x, fs, Hop=512, Window=2048, StartFreq=80.0, StopFreq=1000.0, NumPerOct=48):
...
def cfp_process(
        ...
        win=4096,
        model_type='vocal', 
        num_bins_per_octave = 60
    ):
    if fpath is None and y is None:
        print("Both location and audio array cannot be None")
        return None
    ...
    if 'vocal' in model_type:
        Z, time, CenFreq, tfrL0, tfrLF, tfrLQ = feature_extraction(y, sr, Hop=hop, Window=win, StartFreq=31.0, StopFreq=1250.0, NumPerOct=num_bins_per_octave)
    if 'melody' in model_type:
        Z, time, CenFreq, tfrL0, tfrLF, tfrLQ = feature_extraction(y, sr, Hop=hop, Window=win, StartFreq=20.0, StopFreq=2048.0, NumPerOct=num_bins_per_octave)
